<template>
    <div>
        ...
        <template v-else>
            <h2>
                <b>{{featureResult.clientsQuantity}}</b>&nbsp;<i class="fa primary-color fa-user-o"></i> attented!
            </h2>
        ...
        watch: {
            feature: {
                ...
                handler(newVal, oldVal) {
                    sessionService.getFeatureResult(this.feature.id)
                        .then(result => {
                            this.featureResult = result;
                            this.isLoading = false;
                            this.browsersStats = this.mapStatisticsToChartData(this.featureResult.browsers);
                            this.operatingSystemsStats = this.mapStatisticsToChartData(this.featureResult.systems);
                            this.statusesStats = this.mapStatusesToChartData(this.featureResult.result.statuses);
                        })
                        .catch(reason => logger.error(`Cannot load feature '${this.feature.id}' results`, reason));
                }
            }
        }
    };
